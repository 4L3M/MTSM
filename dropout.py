...
# Model
class DeepCNNWithDropout(nn.Module):
    def __init__(self):
        super(DeepCNNWithDropout, self).__init__()
        self.conv1 = nn.Conv2d(3, 16, kernel_size=3, padding=1)   # 128x128 → 128x128
        self.conv2 = nn.Conv2d(16, 32, kernel_size=3, padding=1)  # 128x128 → 128x128
        self.conv3 = nn.Conv2d(32, 64, kernel_size=3, padding=1)  # 64x64 → 64x64
        self.pool = nn.MaxPool2d(2, 2)  # każdy pooling zmniejsza wymiar o połowę

        self.fc1 = nn.Linear(64 * 16 * 16, 128)  # Po 3 poolingach: 128 → 64 → 32 → 16
...
model = DeepCNNWithDropout().to(device)

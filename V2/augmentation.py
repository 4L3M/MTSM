...
print("________________ augmentation ________________")
"""
    RandomHorizontalFlip() – odbicie lustrzane w poziomie (losowo).

    RandomRotation(10) – obraca obrazy maks. o ±10°.

    ColorJitter() – zmienia jasność i kontrast, symuluje różne warunki obrazowania.

    Resize() – zapewnia spójny rozmiar wejściowy (128×128).

    ToTensor() – konwersja do tensora.
"""

config_name = "Augmented_128x128_SGD_lr0.01"
...
for train_idx, val_idx in skf.split(df['filepath'], df['label']):
    ...
    val_dataset = HistologyDataset(val_df, transform=val_transform)
...
# Zapis do CSV
# Zapis do txt
with open("augment_results.txt", "w", encoding="utf-8") as f:
    for res in results:

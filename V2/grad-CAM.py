...
class DeepCNN(nn.Module):
    ...
    def activate_hook(self, grad):
        self.gradients = grad

    def forward(self, x):
        x = self.pool(F.relu(self.conv1(x)))
        x = self.pool(F.relu(self.conv2(x)))
        x = self.pool(F.relu(self.conv3(x)))  # <-- ostatnia warstwa conv
        h = x.register_hook(self.activate_hook)  # rejestracja hooka
        self.activations = x  # zapis aktywacji
        x = x.view(-1, 64 * 16 * 16)
        x = F.relu(self.fc1(x))
        x = self.fc2(x)
        return x
...
def generate_gradcam(model, image_tensor, class_idx=None):
    ...
    gradients = model.get_activations_gradient().cpu().detach().numpy()[0]
    activations = model.get_activations().cpu().detach().numpy()[0]
...
results_df.to_csv("deepcnn_crossval_results.csv", index=False)
